...
def pcd_maxmin(pcd, print_range=True, device=None):
    # pcd_range = [pcd[..., 0].max().cuda(), pcd[..., 0].min().cuda(),\
    #              pcd[..., 1].max().cuda(), pcd[..., 1].min().cuda(),\
    #              pcd[..., 2].max().cuda(), pcd[..., 2].min().cuda()]
    pcd_range = [pcd[..., 0].max().to(device), pcd[..., 0].min().to(device),\
                 pcd[..., 1].max().to(device), pcd[..., 1].min().to(device),\
                 pcd[..., 2].max().to(device), pcd[..., 2].min().to(device)]
    # if device is not None:
    #     for idx in range(len(pcd_range)):
    #         pcd_range[idx] = pcd_range[idx].to(device)
    if print_range:
        print("X-axis range: {:.4f} ~ {:.4f}", pcd_range[1].item(), pcd_range[0].item())
        print("Y-axis range: {:.4f} ~ {:.4f}", pcd_range[3].item(), pcd_range[2].item())
        print("Z-axis range: {:.4f} ~ {:.4f}", pcd_range[5].item(), pcd_range[4].item())
    return pcd_range
