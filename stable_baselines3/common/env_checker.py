...
def check_env(env: gym.Env, warn: bool = True, skip_render_check: bool = True) -> None:
    ...
    if warn:
        ...
        if isinstance(action_space, spaces.Box) and (np.any(np.abs(action_space.low) != np.abs(action_space.high)) or
                                                     np.any(action_space.low != -1) or np.any(action_space.high != 1)):
            warnings.warn(
