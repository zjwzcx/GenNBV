...
class BasePolicy_Train(BaseModel):
    """The base policy object.
    ...
    def predict(
        ...
        if not th.is_tensor(observation):
            ...
            vectorized_env = True if len(self.observation_space.shape) == 1 else False
...
class BasePolicy_Eval(BaseModel):
    """The base policy object.
    ...
    def obs_to_tensor(self, observation: Union[np.ndarray, Dict[str, np.ndarray]]) -> Tuple[th.Tensor, bool]:
        ...
        obs_tensor = []
        for key, obs in observation.items():
            obs_tensor.append(obs_as_tensor(obs, self.device))
        obs_tensor = th.cat(obs_tensor, dim=1)
...
class ActorCriticPolicy(BasePolicy):
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        if self.ortho_init:
            # NOTE: check for features_extractor
...
class ActorCriticPolicy_Train(BasePolicy_Train):
    """
    Policy class for actor-critic algorithms (has both policy and value prediction).
    Used by A2C, PPO and the likes.
...
class ActorCriticPolicy_Eval(BasePolicy_Eval):
    """
    Policy class for actor-critic algorithms (has both policy and value prediction).
    Used by A2C, PPO and the likes.
