...
class RolloutBuffer(BaseBuffer):
    ...
    def _get_samples(self, batch_inds: np.ndarray, env: Optional[VecNormalize] = None) -> RolloutBufferSamples:
        ...
        return RolloutBufferSamples(*tuple(map(self.to_th, data)))
...
class TensorRolloutBuffer(BaseBuffer):
    def __init__(
        ...
        # else:
        obs_shape = observation_space.shape[0]  # wrapped
    ...
    def reset(self) -> None:
        ...
        #     self.privileged_observations = th.zeros(num_transitions_per_env, num_envs, *privileged_obs_shape, device=self.device)
        # else:
        ...
        super(TensorRolloutBuffer, self).reset()
...
class TensorRolloutBuffer_Grid_Obs(BaseBuffer):
    def __init__(
        self,
        buffer_size: int,
        observation_space: spaces.Space,
    ...
    def reset(self) -> None:
        ...
        self.generator_ready = False
        self.indices = np.random.permutation(self.buffer_size * self.n_envs)
        super(TensorRolloutBuffer_Grid_Obs, self).reset()
...
class DictReplayBuffer(ReplayBuffer):
    ...
    def _get_samples(self, batch_inds: np.ndarray, env: Optional[VecNormalize] = None) -> DictReplayBufferSamples:
        ...
        observations = {key: self.to_th(obs) for key, obs in obs_.items()}
        next_observations = {key: self.to_th(obs) for key, obs in next_obs_.items()}

        return DictReplayBufferSamples(
            observations=observations,
            actions=self.to_th(self.actions[batch_inds, env_indices]),
            next_observations=next_observations,
            # Only use dones that are not due to timeouts
            # deactivated by default (timeouts is initialized as an array of False)
            dones=self.to_th(self.dones[batch_inds, env_indices] * (1 - self.timeouts[batch_inds, env_indices])
                             ).reshape(-1, 1),
            rewards=self.to_th(self._normalize_reward(self.rewards[batch_inds, env_indices].reshape(-1, 1), env)),
        )
...
class DictRolloutBuffer(RolloutBuffer):
    ...
    def _get_samples(self, batch_inds: np.ndarray, env: Optional[VecNormalize] = None) -> DictRolloutBufferSamples:
        ...
        return DictRolloutBufferSamples(
            observations={key: self.to_th(obs[batch_inds])
                          for (key, obs) in self.observations.items()},
            actions=self.to_th(self.actions[batch_inds]),
            old_values=self.to_th(self.values[batch_inds].flatten()),
            old_log_prob=self.to_th(self.log_probs[batch_inds].flatten()),
            advantages=self.to_th(self.advantages[batch_inds].flatten()),
            returns=self.to_th(self.returns[batch_inds].flatten()),
        )
