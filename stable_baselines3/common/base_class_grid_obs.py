...
from stable_baselines3.common.callbacks import BaseCallback, CallbackList, ConvertCallback, EvalCallback, EvalCallback_Grid_Obs
...
class BaseAlgorithm_Grid_Obs(ABC):
    """
    ...
    def _init_callback(
        ...
        if eval_env is not None:
            eval_callback = EvalCallback_Grid_Obs(
                eval_env,
    ...
    def _setup_learn(
        ...
        if reset_num_timesteps or self._last_obs is None:
            self._last_obs = self.env.reset()   # NOTE: reset(): initial action and visual observations
        ...
        # keep same with PPO from RSL-rl
        if is_isaac_gym_env(self.env):
            self.env.episode_length_buf = th.randint_like(
    ...
    def _update_info_buffer(
        self, infos: List[Dict[str, Any]], dones: Optional[np.ndarray] = None, is_isaac_gym=False
    ) -> None:
        """
